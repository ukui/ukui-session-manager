...
enum {
        ...
        GSM_CONDITION_UKUI          = 3,
        GSM_CONDITION_GSETTINGS     = 4,
        GSM_CONDITION_UNKNOWN       = 5
};

#define GSM_SESSION_CLIENT_DBUS_INTERFACE "org.ukui.SessionClient"
...
is_disabled (GsmApp *app)
{
        ...
        /* GSM_AUTOSTART_APP_ENABLED_KEY key, used by old ukui-session */
                        ...
                        char      **keyp)
{
        ...
        } else if (!g_ascii_strncasecmp (condition_string, "UKUI", len)) {
                kind = GSM_CONDITION_UKUI;
...
setup_condition_monitor (GsmAutostartApp *app)
{
        ...
        if (kind == GSM_CONDITION_IF_EXISTS) {
                ...
                g_object_unref (file);
                g_free (file_path);
        } else if (kind == GSM_CONDITION_UKUI) {
...
is_conditionally_disabled (GsmApp *app)
{
        ...
        if (kind == GSM_CONDITION_IF_EXISTS) {
                ...
                disabled = g_file_test (file_path, G_FILE_TEST_EXISTS);
                g_free (file_path);
        } else if (kind == GSM_CONDITION_UKUI && priv->condition_settings != NULL) {
                char **elems;
                elems = g_strsplit (key, " ", 2);
