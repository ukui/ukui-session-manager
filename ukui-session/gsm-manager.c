...
manager_perhaps_lock (GsmManager *manager)
{
        ...
        /* only lock if ukui-screensaver is set to lock */
        ...
        error = NULL;
        ret = g_spawn_command_line_sync ("ukui-screensaver-command --lock", NULL, NULL, NULL, &error);
        if (!ret) {
...
gsm_manager_init (GsmManager *manager)
{
        ...
        /* check if ukui-screensaver is installed */
...
request_reboot (GsmManager *manager)
{
        ...
        if (LOGIND_RUNNING()) {
                ...
                if (!success) {
                        ...
                        priv->logout_type = GSM_MANAGER_LOGOUT_REBOOT_UKDM;
                        end_phase (manager);
                }
        }
        else {
...
request_shutdown (GsmManager *manager)
{
        ...
        if (LOGIND_RUNNING()) {
                ...
                if (!success) {
                        ...
                        priv->logout_type = GSM_MANAGER_LOGOUT_SHUTDOWN_UKDM;
                        end_phase (manager);
                }
        }
        else {
